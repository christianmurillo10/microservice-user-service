...
import { EVENT_ORGANIZATION, EVENT_ORGANIZATION_BULK_DELETED, EVENT_ORGANIZATION_CREATED, EVENT_ORGANIZATION_DELETED, EVENT_ORGANIZATION_UPDATED } from "../../shared/constants/events.constant";
...
export default class OrganizationKafkaProducer {
  ...
  organizationCreatedEventEmitter = async (data: EventMessageData<OrganizationEntity>, userId: string, headers?: IHeaders): Promise<void> => {
    ...
    await this.kafkaService.initializeProducer(EVENT_ORGANIZATION, EVENT_ORGANIZATION_CREATED, data, userId, headers);
  ...
  organizationUpdatedEventEmitter = async (data: EventMessageData<OrganizationEntity>, userId: string, headers?: IHeaders): Promise<void> => {
    await this.kafkaService.connectProducer();
    await this.kafkaService.initializeProducer(EVENT_ORGANIZATION, EVENT_ORGANIZATION_UPDATED, data, userId, headers);
    await this.kafkaService.disconnectProducer();
  };
  ...
  organizationDeletedEventEmitter = async (data: EventMessageData<OrganizationEntity>, userId: string, headers?: IHeaders): Promise<void> => {
    await this.kafkaService.connectProducer();
    await this.kafkaService.initializeProducer(EVENT_ORGANIZATION, EVENT_ORGANIZATION_DELETED, data, userId, headers);
    await this.kafkaService.disconnectProducer();
  };

  organizationBulkDeletedEventEmitter = async (data: EventMessageData<unknown>, userId: string, headers?: IHeaders): Promise<void> => {
    await this.kafkaService.connectProducer();
    await this.kafkaService.initializeProducer(EVENT_ORGANIZATION, EVENT_ORGANIZATION_BULK_DELETED, data, userId, headers);
    await this.kafkaService.disconnectProducer();
  };
};